...
class GraphiantCoreLanInterface(object):
    ...
    def __init__(self):
        ...
        self.core = Core(parameters['base_url'], parameters['username'], parameters['password'])


def run_module():
    class_object = GraphiantCoreLanInterface()
    if class_object.action.lower() in ["deconfig", "deconfigure", "delete"]:
        status = class_object.core.delete_interface(**class_object.interface_data)
    else:
        status = class_object.core.add_core_interface(**class_object.interface_data)
    if status:
