...
class GraphiantEdgeLanSubInterface(object):
...
def run_module():
    class_object = GraphiantEdgeLanSubInterface()
    if class_object.action.lower() in ["deconfig", "deconfigure", "delete"]:
        status = class_object.edge.delete_subinterface(**class_object.interface_data)
    else:
        status = class_object.edge.add_lan_subinterface(**class_object.interface_data)
    if status:
