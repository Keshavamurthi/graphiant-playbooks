...
class EdgeTemplates(object):
    ...
    def render_template(self, config_template_file, **kwargs):
        """
        Renders config_template_file(Jinja2 template) with the provided variables and returns the parsed YAML.
    ...
    def _edge_interface(self, **kwargs):
        ...
        Args:
            **kwargs: Key-value pairs used to populate the interface template.

        Returns:
            dict: Parsed YAML configuration for interface settings.
        """
    ...
    def _global_bgp_filter(self, **kwargs):
        """
        Renders the 'global_bgp_routing_policies_template.yaml' Jinja2 template with the provided variables.
